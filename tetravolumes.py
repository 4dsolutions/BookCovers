...
class Tetrahedron:
    ...
    def __init__(self, a,b,c,d,e,f, pv = 1):
    ...
    def ivm_volume(self):
        ivmvol = (((self._addopen()- self._addclosed() - self._addopposite())/2) ** 0.5)/self.pv**3
...
class Test_Tetrahedron(unittest.TestCase):
    ...
    def test_unit_volume2(self):
        ...
        self.assertAlmostEqual(tet.xyz_volume(), 0.11785, "Volume not expected")

    def test_phi_edge_tetra(self):
        tet = Tetrahedron(D, D, D, D, D, PHI)
        self.assertAlmostEqual(tet.ivm_volume(), 0.70711, "Volume not expected")
